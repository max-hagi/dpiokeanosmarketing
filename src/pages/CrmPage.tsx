...
import {
  ...
  Brain, Trophy, XCircle, Phone, Mail, ArrowRight, Loader2, RotateCw,
  AlertTriangle, Target, Sparkles, MessageSquare
} from "lucide-react";
...
const stagePills = ["Inquiry", "Quoted", "Proposal Sent", "Negotiating", "Won", "Lost"];
...
export default function CrmPage() {
  ...
  const { data: followUpMessages } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("follow_up_messages").select("*").order("created_at", { ascending: true });
      if (error) throw error;
      return data;
    },
  });
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold capitalize ${
                        r.lead_stage === "quoted" ? "bg-accent/10 text-accent" :
                        r.lead_stage === "qualified" ? "bg-primary/10 text-primary" :
                      ...
                      <div className="space-y-1.5">
                        {msgs.map(m => (
                          <div key={m.id} className="flex items-center gap-2 text-xs">
                            ...
                            <span className="text-muted-foreground">{m.sent_at ? format(new Date(m.sent_at), "MMM d, h:mm a") : "Pending"}</span>
                            <span className="flex-1 truncate">Msg {m.message_number}: {m.subject}</span>
                            <span className={`text-[9px] font-bold uppercase ${m.status === "sent" ? "text-success" : "text-muted-foreground"}`}>{m.status}</span>
                          </div>
                        ))}
                      </div>
                    </section>
